...
export function analyzeAssetReadings({ readings = [], recentFailures = 0, ageYears = 0 }) {
  ...
  const anomalyCount = readings.filter((v) => v > anomalyThreshold).length;
  const trendSlope = readings[readings.length - 1] - readings[0];

  const rawRiskScore =
    0.4 * (anomalyCount / readings.length) +
  ...
  return { riskLevel, failureProbability, anomalyCount, trendSlope, recommendation };
}
